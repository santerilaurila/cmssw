...
def keepSelected(dataTier):
	 ret_vstring = cms.untracked.vstring(
			 ...
			 "keep *_selectedMuonForSingleMuonEmbedding_*_"+dataTier,
...
def customiseSelecting(process,reselect=False):
	...
	process.load('TauAnalysis.MCEmbeddingTools.SelectingProcedure_SingleTau_cff')
	process.selectedMuonForSingleMuonEmbedding.src = cms.InputTag("slimmedMuons","",dataTier)

	process.selecting = cms.Path(process.makePatMuonsForSingleTauEmbedding)
...
def customiseCleaning(process, changeProcessname=True,reselect=False):
	...
	MuonImput = cms.InputTag("selectedMuonForSingleMuonEmbedding","","")  ## This is the muon (in ditau embedding, the parameter is selectedMuonsForEmbedding)
	for akt_manimod in to_bemanipulate:
		if "CLEAN" in akt_manimod.steps:
...
def customiseLHE(process, changeProcessname=True,reselect=False):
	...
	process.load('TauAnalysis.MCEmbeddingTools.SingleTauEmbeddingLHEProducer_cfi')
	if reselect:
